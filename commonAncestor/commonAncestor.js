...
/**
  ...
  */
Tree.prototype.getClosestCommonAncestor = function(child1, child2, ancestor) {
...
Tree.prototype.getAncestorPath = function(child, path) {
  ...
  for (var i = 0; i < this.children.length; i++) {
    if (this.children[i].isDescendant(child) || this.children[i] === child) {
      path.push(this.children[i]);
      this.children[i].getAncestorPath(child, path);
    }
  }

...
BinaryHeap.prototype.insert = function (value) {
  ...
  let startIndex = this._heap.length - 1;
  let parentIndex = Math.floor( (startIndex - 1) / 2 );
  if(parentIndex > -1 && this._compare(this._heap[startIndex], this._heap[parentIndex])) {
    let isValueGreater = true;
    while (isValueGreater) {
      let parent = this._heap[parentIndex];
      this._heap[parentIndex] = value;
      this._heap[startIndex] = parent;
      startIndex = parentIndex;
      parentIndex = Math.floor( (startIndex - 1) / 2 );
      if (!this._compare(this._heap[startIndex], this._heap[parentIndex])) {
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  let lastNode = this._heap[lastIndex];
  this._heap[lastIndex] = this.getRoot();
  ...
  if (this.getRoot() > child1 || this.getRoot() > child2) {

    let isValueGreater = true;
    let swapValueIndex = 0;

    while (isValueGreater) {
      let child1Index = swapValueIndex * 2 + 1;
      let child2Index = swapValueIndex * 2 + 2;

      let minChildIndex = this._compare(this._heap[child2Index], this._heap[child1Index]) ? child2Index : child1Index;

      let temp = this._heap[minChildIndex];
      this._heap[minChildIndex] = this._heap[swapValueIndex];
      this._heap[swapValueIndex] = temp;
      swapValueIndex = minChildIndex;

      child1Index = swapValueIndex * 2 + 1;
      child2Index = swapValueIndex * 2 + 2;
      if (!this._compare(this._heap[child1Index], this._heap[swapValueIndex]) && !this._compare(this._heap[child2Index], this._heap[swapValueIndex])) {
        isValueGreater = false;
      }
    }
  }
